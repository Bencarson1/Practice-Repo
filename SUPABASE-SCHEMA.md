...
## Added by wearvia/supabase/worldwide.sql

- **currencies** (code, name, symbol, decimals, trim_zeros, sort_order) — everyone reads; **exchange_rates** (currency_code, units_per_usd, rate_date, source, updated_at) — everyone reads, only `wearvia_save_exchange_rates(rates, date, source)` writes, and only the secret key (service_role) may call it (the GitHub Action "Exchange rates").
- **countries** gains currency_code, phone_code (+44) and fabric_unit ('yd' in GB, NG, GH, US; 'm' elsewhere).
- **currency_code** on designers, suppliers, price_list, fabrics, orders, invoices, payments, fabric_order_lines, ready_to_wear_items and ready_to_wear_sales. Rows from before are GBP. Triggers fill it: a tailor or seller from their country; a fabric from its seller; a price from its tailor; payments and invoices from their order; a seller's order line from the seller.
- **suppliers** gains country_code. **customers** gains country_code, currency_code and measurement_unit ('in' or 'cm'; measurements stay in inches).
- **orders** gains fabric_currency_code, fabric_price_per_yard and fabric_cost_in_fabric_currency (the seller's side), exchange_rate (1 in the seller's currency = this in the order's), exchange_rate_date, exchange_rate_source and fabric_unit. The app can't change them (`wv_orders_guard_currency`).
- `wearvia_send_quote(order, length, fabric, note, unit)` prices the quote in the tailor's currency and converts the seller's fabric at today's rate (unit 'yd' or 'm'); `wearvia_accept_quote`, walk-in orders (`wv_orders_before_insert`) and `wv_order_take_fabric` keep the seller's line in the seller's currency and the invoice in the order's. `wearvia_convert_price_list(designer)` puts a tailor's prices into their currency; changing a tailor's or seller's currency converts their prices. `wv_money_text(amount, currency)` writes money (£1,250 · ₦250,000 · $1,250.00). `wearvia_tailor_page` also returns currency_code, fabric_unit and from_price.
