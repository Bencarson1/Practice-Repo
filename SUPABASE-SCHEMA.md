...
This is the live layout of the existing Supabase project. Build on it — do not drop or recreate these tables. Add new columns/tables with `alter table ... add column if not exists` / `create table if not exists`.
...
- **fabrics**: id uuid, supplier_id uuid, name text, category text, price_per_metre numeric, metres_available numeric, min_order_metres numeric, created_at timestamp with time zone
